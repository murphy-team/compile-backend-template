pipeline {
    ...
    stages {
        ...
        stage('Prepare Environment') {
            steps {
                git branch: '$BRANCH_NAME', url: 'git@gitlab.example.com:group-name/compile-backend-template.git'
            }
        }
            ...
            steps {
                build job: 'deployment-pipeline',
                        parameters: [[$class: 'StringParameterValue', name: 'PROJECT_WORKSPACE', value: "${env.WORKSPACE}"],
                                     ...
                                     [$class: 'StringParameterValue', name: 'ARTIFACT_NAME', value: "compile-backend-template-ear-1.0.0.ear"]]
            }
        }
    }
}
